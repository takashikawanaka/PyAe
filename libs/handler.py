...
class Handler(PatternMatchingEventHandler):
    ...
    def __init__(
        ...
        self.__cache_file_hash: dict[str, str] = {}
    ...
    def on_moved(self, event: FileMovedEvent) -> None:
        ...
        self.__cache_file_hash.pop(event.src_path)
        self.__set_file_hash(path=event.dest_path)
        self.__process.execute()

    def on_created(self, event: FileCreatedEvent) -> None:
        print(f"File created, {event.src_path}")
        # 新規作成
        self.__set_file_hash(path=event.src_path)
        self.__process.execute()
    ...
    def on_modified(self, event: FileModifiedEvent) -> None:
        ...
        if hash != self.__cache_file_hash[filepath]:
            # ファイルに変更があった場合、書き換え
            print(f"File modified, {event.src_path}")
            self.__cache_file_hash[filepath] = hash
            self.__process.execute()
    ...
    def __set_file_hash(self, path: str) -> str:
        self.__cache_file_hash[path] = self.__get_file_hash(path=path)
